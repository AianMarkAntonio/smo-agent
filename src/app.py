...
#             url="https://dsb1qxkgnj.execute-api.ap-southeast-1.amazonaws.com/dev/chat/smo-agent",#settings.API_URL,
#             json={"prompt":prompt},
#             headers=headers
#         )
#         print(response.json())
...
def fetch_bedrock_response(prompt):
    """
    Call the Bedrock API and return the JSON response.
    """
    try:
        headers = {
            "x-api-key": "koStY9EEHaaLwi2mL7FvW9xO3q9xnoaS9HTjzMup",  # replace with settings.API_KEY
            "Content-Type": "application/json"
        }
        response = requests.post(
            url="https://dsb1qxkgnj.execute-api.ap-southeast-1.amazonaws.com/dev/chat/smo-agent",  # replace with settings.API_URL
            json={"prompt": prompt},
            headers=headers
        )
...
def chat_with_kb(user_input):
    """
    Generate a response from Bedrock and update chat history.
    """
    # Build conversation context (last 10 turns)
    context = ""
    for msg in st.session_state.chat_history[-10:]:
    ...
    if raw and raw.status_code == 200:
        ...
        # Try extracting "response" first
        response = raw_json.get("response")
        if response:
            answer = response.get("output", {}).get("text", answer)
            citation = response.get("citations", [])
            if citation:
                location = citation[0].get("retrievedReferences", [{}])[0].get("location", {})
                sql_query = location.get("sqlLocation", {}).get("query", "")
        else:
            # Fallback to "answer" key
            answer = raw_json.get("answer", answer)
...
for msg in st.session_state.chat_history:
    if msg["role"] == "user":
        ...
        if msg.get("corresponding_query"):
            with st.expander("🛢️ Corresponding Query"):
                st.caption(msg["corresponding_query"])
